...
const ESP32S3Romeo = ({ node = {}, nodeData = {}, topics = {}, color }: any) => {
    const { id } = node
...
export default {
    ...
    check: (node, nodeData) => node.type == "ArrayLiteralExpression" && nodeData['element-1'] == '"ESP32 S3 Romeo"',
    getComponent: (node, nodeData, children) => <ESP32S3Romeo color={getColor('ESP32S3Romeo')} node={node} nodeData={nodeData} children={children} />,
    getInitialData: () => { return { to: '"ESP32S3Romeo"' } },
    hidden: true,
    nonDeletable: true
}